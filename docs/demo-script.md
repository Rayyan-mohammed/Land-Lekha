...
- **Dashboard**: documents processed, auto-accept rate, pending verification, error statistics, state- and district-wise progress - including a live map, circles sized by volume and coloured by share digitized - benchmark CER and field accuracy, and corrections per 100 fields checked - which is counted from what verifiers actually changed, not assumed.
...
- On a verified record, press **Check against the state register**. The card lists, field by field, what
  this document says against what the register holds - here the owner differs, because the paper predates a
  mutation. Say the honest line out loud: the register is **simulated**, and a match means the two records
  agree, not that the document is genuine. This is the check a tehsildar does by hand today.
