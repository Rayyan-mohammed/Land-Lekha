  ...
  return <div>
    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_320px] mt-3">
        ...
        {!users ? <div className="p-10 flex justify-center"><Spinner /></div> :
          <div className="table-wrap"><table className="data">
            ...
            <tbody>{users.map((u) => <tr key={u.id}>
              ...
              <td><select className="input w-auto py-1" value={u.role} disabled={u.id === me.id} onChange={(e) => update(u, { role: e.target.value })}>
                {Object.entries(ROLE_LABEL).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select></td>
              <td>{u.active ? <span className="text-ok text-sm">Active</span> : <span className="text-slate-400 text-sm">Disabled</span>}</td>
      ...
      <form onSubmit={create} className="card p-4 space-y-3 h-fit">
        ...
        <div><label className="label">Role</label><select className="input" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })}>
          {Object.entries(ROLE_LABEL).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select></div>
        <div><label className="label">Password (min 8)</label><input type="password" className="input" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} /></div>
