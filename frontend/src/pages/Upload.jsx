...
export default function UploadPage() {
  ...
  return <div>
    <PageHeader title="Upload land records" subtitle="Scanned PDFs, images or phone photos · Hindi and English · printed or handwritten" />
    <div
      ...
      <p className="mt-3 font-medium text-slate-800">Drop files here</p>
      <p className="text-sm text-slate-500">PNG, JPG, TIFF or PDF, up to 20 MB each</p>
      <div className="mt-5 flex flex-wrap justify-center gap-2">
        <button className="btn-primary" onClick={() => fileRef.current.click()}><FileUp size={16} /> Choose files</button>
        <button className="btn-outline sm:hidden" onClick={() => camRef.current.click()}><Camera size={16} /> Take photo</button>
      </div>
        ...
        return <div key={x.key} className="flex flex-wrap items-center gap-3 px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-xs text-slate-500">
              {x.error ? <span className="text-bad">{x.error}{x.dupId && <> — <Link className="underline" to={`/documents/${x.dupId}`}>open existing</Link></>}</span>
                : !d ? 'Uploading…'
                  : !done ? 'Reading document (preprocessing → OCR → field extraction)…'
          ...
          {done && <Link to={`/documents/${d.id}`} className="btn-outline py-1.5">Open</Link>}
        </div>
      })}
    </div>}
  </div>
