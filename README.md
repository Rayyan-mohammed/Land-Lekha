...
- **The data is almost entirely synthetic.** No public labelled dataset of Indian land records exists, so every headline number comes from a generator (`data/generator/`). The first three real documents score 18.2% of fields ([eval/results/real.md](eval/results/real.md)) - too few to call an accuracy, enough to show where the system breaks on real paper and that it fails safely.
